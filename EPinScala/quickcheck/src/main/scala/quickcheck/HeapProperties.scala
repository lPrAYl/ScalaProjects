...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val meldedHeap: List[Node] =
        val heapMax = insert(max, insert(max, empty))
        val heapMin = insert(min, insert(min, empty))
        meld(heapMax, heapMin)
  ...
  val meldingHeaps: (String, Prop) =
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      def go(h1: List[Node], h2: List[Node], meldedHeap: List[Node]): Boolean =
        if h1.isEmpty && h2.isEmpty && meldedHeap.isEmpty then true
        else if ((heap1.nonEmpty || heap2.nonEmpty) && meldedHeap.isEmpty)
          || ((heap1.isEmpty && heap2.isEmpty) && meldedHeap.nonEmpty) then false
        else
          lazy val min: Int = findMin(meldedHeap)
          if h1.nonEmpty && findMin(h1) == min then go(deleteMin(h1), h2, deleteMin(meldedHeap))
          else go(h1, deleteMin(h2), deleteMin(meldedHeap))

      go(heap1, heap2, meld(heap1, heap2))
    }
