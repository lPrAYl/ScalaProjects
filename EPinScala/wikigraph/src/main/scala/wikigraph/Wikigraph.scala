...
final class Wikigraph(client: Wikipedia):
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty then WikiResult.successful(None)
        ...
        val ((dist, articleId), tailQueue) = q.dequeue
        if dist >= maxDepth then WikiResult.successful(None)
        else
          client.linksFrom(articleId).flatMap { articleIds =>
            if articleIds.contains(target) then WikiResult.successful(Some(dist))
            else
              val toVisit = (articleIds -- visited).map((dist + 1, _))
              iter(visited + articleId, tailQueue.enqueueAll(toVisit))
          }.fallbackTo(iter(visited + articleId, tailQueue))
    end iter
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    val articlePairs = for {
        source <- titles
        target <- titles
        if source != target
      } yield (source, target)

    WikiResult.traverse[(String, String), (String, String, Option[Int])](articlePairs) {
      (sourceTitle, targetTitle) =>
        client.searchId(sourceTitle)
          .zip(client.searchId(targetTitle))
          .flatMap {
            (sourceId, targetId) =>
              breadthFirstSearch(sourceId, targetId, maxDepth)
                .flatMap {
                  distance =>
                    WikiResult.successful(sourceTitle, targetTitle, distance)
                }
          }
    }
end Wikigraph
